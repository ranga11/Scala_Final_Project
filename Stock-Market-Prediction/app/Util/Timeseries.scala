...
object Timeseries extends App{
    ...
    def trainAndPredictPrice():Array[String]={
        ...
        val formattedData = data
          .flatMap{
              row =>
                  Array(
                      (row.getString(row.fieldIndex("appleDate")), "apple", row.getString(row.fieldIndex("closeApple"))),
                      (row.getString(row.fieldIndex("appleDate")), "amazon", row.getString(row.fieldIndex("closeAmazon"))),
                      (row.getString(row.fieldIndex("appleDate")), "facebook", row.getString(row.fieldIndex("closefacebook"))),
                      (row.getString(row.fieldIndex("appleDate")), "google", row.getString(row.fieldIndex("closegoogle"))),
                      (row.getString(row.fieldIndex("appleDate")), "microsoft", row.getString(row.fieldIndex("closemicrosoft")))
                  )
    ...
    def getActualPrice(): Array[Array[Double]] = {
        ...
        val actualPrices =Array(applePriceActual1,amazonPriceActual1,googlePriceActual1,microsoftPriceActual1,facebookPriceActual1)
